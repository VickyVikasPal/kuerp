...
import lang from "@/views/modules/Invoices/language/en";3
...
export default {
    ...
    methods: {
        saveProduct() {
            const self = this;
            self.loading = true;
            axios.post('/modules/invoices', self.product).then(function (response) {
                self.loading = false;
                self.$notify({
                ...
                self.$router.push('/modules/invoices/' + response.data.product.id + '/edit');
            }).catch(function () {
                self.loading = false;
            });
        },
        getProducts() {
           ...
           axios.get('/modules/invoices/'+self.$route.params.id).then(function(response){
              ...
              self.invoice.client_id = response.data.client_master.id;
              self.invoice.client_type = 'clientmasters';
              self.invoice.client_name = response.data.client_master.name;
        ...
        addProduct(){
            ...
            let client_id = self.invoice.client_id;
            let invoice_id = self.$route.params.id;
           
            let discount = document.getElementById('discount').value;
            let product_qty = document.getElementById('product_qty').value;
           
            let purchase_obj = {'invoice_id':invoice_id, 'product_id':product_id, 'client_id':client_id, 'discount':discount, 'product_qty':product_qty}
        
            //purchase_id
            axios.post('modules/invoices/updateitem', purchase_obj).then(function(response){
        ...
        delRecord(id){
            ...
            self.loading = true;
            self.productArray = [];
            axios.post('modules/del_invoice', {'del_id':id}).then(function(response){
                if(response.data.message == "success"){
        ...
        {
            const self = this;
            self.invoice_id = self.$route.params.id;
            if(action =='save')
            {
               let saveObj = {
                   'action':action,
                ...
                axios.post('modules/invoices/updateinvoice',saveObj).then(function(response){
                    self.$router.push('/modules/invoices/'+self.invoice_id+'/detail');
                })
            }
