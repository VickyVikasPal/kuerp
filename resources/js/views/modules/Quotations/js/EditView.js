...
import lang from "@/views/modules/Quotations/language/en";3
...
export default {
    ...
    methods: {
        saveProduct() {
            const self = this;
            self.loading = true;
            axios.post('/modules/quotations', self.product).then(function (response) {
                self.loading = false;
                self.$notify({
                ...
                self.$router.push('/modules/quotations/' + response.data.product.id + '/edit');
            }).catch(function () {
                self.loading = false;
            });
        },
        ...
        addProduct(){
            ...
            let quotation_id = self.$route.params.id;
           
            let discount = document.getElementById('discount').value;
            let product_qty = document.getElementById('product_qty').value;
           
            let purchase_obj = {'quotation_id':quotation_id, 'product_id':product_id, 'client_id':client_id, 'discount':discount, 'product_qty':product_qty}
        
            //purchase_id
            axios.post('modules/quotations', purchase_obj).then(function(response){
        ...
        {
            const self = this;
            self.quotation_id = self.$route.params.id;
            if(action =='save')
            {
               let saveObj = {
                   'action':action,
                ...
                axios.post('modules/updatequotation',saveObj).then(function(response){
