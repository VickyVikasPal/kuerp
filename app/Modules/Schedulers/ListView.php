...
class ListView
{
    public function getDatas($request)
    {
        ...
        try
        {
            ...
            if (count($dateArr) > 0)
            {
                ...
                $items = Scheduler::where($next_array)
            ...
            $scheduler->LogDebug('End: Schedulers.ListView->getDatas()', ['count' => count($items)]);

            $array = [
                'items' => SchedulerResource::collection($items->items()),
                'pagination' => [
        ...
        {
            $scheduler->LogCritical('Failed: Schedulers.ListView->getDatas()', ['error' => $e->getMessage()]);
    ...
    public function export($request)
    {
        ...
        try
        {
            ...
            $scheduler->LogDebug('End: Schedulers.ListView->export()', ['count' => count($items)]);

            $array = SchedulerResource::collection($items->items());
        ...
        catch (\Throwable $e)
        {
            ...
            $scheduler->LogCritical('Failed: Schedulers.ListView->export()', ['error' => $e->getMessage()]);
