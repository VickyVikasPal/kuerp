    ...
    public function toArray($request)
    {
      ...
      //  print_r($user_pref_data);die;
        return [
            ...
            'avatar' => $user->avatar,
            'avatar_path' => '/uploads/'.date('Y',strtotime($user->date_entered)).'/'.date('m',strtotime($user->date_entered)).'/'.$user->avatar,
            'user_type' => $user->user_type,
            'status' => $user->status == '1' ? 'Active' : 'Inactive',
            'date_entered' => $user->date_entered ? $datetime->to_display_date_time($this->date_entered) : '',
            'date_modified' => $user->date_entered ? $datetime->to_display_date_time($this->date_modified) : '',
            'updated_at' => $user->date_modified ? $datetime->to_display_date_time($this->updated_at) : '',
