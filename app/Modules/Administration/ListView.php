...
use App\Modules\Administration\Resources\AdministrationResource;
...
class ListView
{
    public function getDatas($request)
    {
        ...
        if (count($dateArr) > 0)
        {
            ...
            $items = Administration::where($next_array)
        ...
        $array = [
            'items' => AdministrationResource::collection($items->items()),
            'pagination' => [
    ...
    public function getAdministrationById($parent_id)
    {
        $data = Administration::where('parent_id', '=', $parent_id)->where('deleted', '=', '0')->get();
        return $data;
    }
    ...
    public function export($request)
    {
        ...
        $array = AdministrationResource::collection($items->items());
        return json_encode($array, true);
    }

    public function getActiveAdministration()
    {
        $data = Administration::where('status', '=', 'Active')->where('deleted', '=', '0')->get();
        return $data;
    }
}