...
class ListView
{
    public function getDatas($request)
    {
        ...
        try
        {
            ...
            if (count($dateArr) > 0)
            {
                ...
                $items = CountryList::where($next_array)
            ...
            $countrylist->LogDebug('End: CountryLists.ListView->getDatas()', ['count' => count($items)]);

            $array = [
                'items' => CountryListResource::collection($items->items()),
                'pagination' => [
    ...
    public function export($request)
    {
        ...
        try
        {
            ...
            $array = CountryListResource::collection($items->items());
            return json_encode($array, true);
        }
        catch (\Throwable $e)
        {
            // Log a critical error message
            $countrylist->LogCritical('Failed to export records', ['error' => $e->getMessage()]);
