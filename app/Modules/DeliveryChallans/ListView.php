    ...
    public function get_list($request){
     ...
     // print_r($next_array);
       ...
       /// $sort = json_decode($request->get('sort', json_encode(['order' => 'desc', 'column' => 'created_at'], JSON_THROW_ON_ERROR)), true, 512, JSON_THROW_ON_ERROR);
            ...
            $items = DeliveryChallan::where($next_array)
        ...
        $array =   [
        'items' => DeliveryChallanResource::collection($items->items()),
        'pagination' => [
...
{
     $purchase_data = DeliveryChallan::find($request->delivery_id);
